...
namespace APIPeli.Filtros
{
    public class FiltroValidaciones<T> : IEndpointFilter
    {
        ...
        {
            var validador = contexto.HttpContext.RequestServices.GetService<IValidator<T>>(); //es la manera de obtener un servicio con código
            ...
            var insumoAValidar = contexto.Arguments.OfType<T>().FirstOrDefault();
